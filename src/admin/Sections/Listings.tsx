...
const Listings = () => {
  ...
  const sendData = async () => {
    ...
    try {
      const res = await fetch(
        `https://e-shop-wepb.onrender.com/${route}/update/${id}`,
        {
          method: "PATCH",
          headers: {
  ...
  const getData = async () => {
    try {
      const res = await fetch(
        `https://e-shop-wepb.onrender.com/${route}?category=${category}&search=${search}&page=${page}&limit=10`
      );
      const data = await res.json();
  ...
  const handleVisibility = async (el: any) => {
    ...
    try {
      const res = await fetch(
        `https://e-shop-wepb.onrender.com/${route}/update/${el._id}`,
        {
          method: "PATCH",
          headers: {
            "Content-Type": "application/json",
